...
def abrirHTMLGenerado(textAreaInicial):
    texto = textAreaInicial.get("1.0", "end")
    lexemas = []
    errores = []
    caracteres_invalidos = ""
    ...
    # Aquí puedes hacer lo que desees con los lexemas, errores y caracteres inválidos
    
    # Retorna el texto limpio de caracteres inválidos
    texto_limpio = texto
    for char in caracteres_invalidos:
...
def Traductor():
    ...
    ttk.Button(buttonFrame, text="Abrir HTML Generado", command=lambda: abrirHTMLGenerado(textAreaInicial)).grid(row=0, column=2, padx=5, pady=5)
